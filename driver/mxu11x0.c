...
//CJ	ret = usb_serial_register_drivers(&mxu1_usb_driver, serial_drivers);
...
#else
		//CJ usb_serial_debug_data(debug, dev, __FUNCTION__,
...
#else
    struct tty_port *tty;
    /* for Linux 3.16, tty should be tty_port* */
...
static void mxu1_send(struct mxu1_port *mxport)
{
	...
	spin_unlock_irqrestore(&mxport->mxp_lock, flags);
#if(LINUX_VERSION_CODE < KERNEL_VERSION(2,6,9))
	usb_serial_debug_data(&port->port_number, __FUNCTION__, count, port->write_urb->transfer_buffer);
#else
	usb_serial_debug_data(&port->serial->dev->dev, __FUNCTION__, count, port->write_urb->transfer_buffer);
#endif
